...
\else % if luatex or xelatex
	...
	$if(mathfont)$ \setmathfont{$mathfont$} $endif$
\fi
...
\hrule width \hsize height 0pt \kern 1mm \hrule width \hsize   
\end{minipage}\par}%

\renewcommand\headrule{
\begin{minipage}{1\textwidth}
\hrule width \hsize \kern 1mm \hrule width \hsize height 0pt 
\end{minipage}}%
...
\fancyfootoffset{0pt}
\fancyfoot[L]{Maxime HORCHOLLE \& Cédric TESNIERE}
\fancyfoot[R]{\thepage\ sur {\hypersetup{linkcolor=black}\pageref{LastPage}}}
...
$if(lhs)$ \lstnewenvironment{code}{\lstset{language=Haskell,basicstyle=\small\ttfamily}}{} $endif$
$if(highlighting-macros)$ $highlighting-macros$ $endif$
$if(verbatim-in-note)$ \usepackage{fancyvrb} $endif$
$if(tables)$ \usepackage{longtable} $endif$
$if(graphics)$
	\usepackage{graphicx}
